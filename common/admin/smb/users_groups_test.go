...
func TestUsersAndGroupsIdentity(t *testing.T) {
	cluster := NewUsersAndGroups("ug1")
	assert.Equal(t, cluster.Type(), UsersAndGroupsType)
	cid := cluster.Identity()
	assert.Equal(t, cid.Type(), UsersAndGroupsType)
	assert.Equal(t, cid.String(), "ceph.smb.usersgroups.ug1")
