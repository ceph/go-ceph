...
func TestMarshalUnmarshalShare(t *testing.T) {
	s := NewShare("floop", "boop").SetCephFS("cephfs", "g1", "v1", "/")
	...
	assert.EqualValues(t, s.ClusterID, s2.ClusterID)
	assert.EqualValues(t, s.ShareID, s2.ShareID)
	assert.EqualValues(t, s.IntentValue, s2.IntentValue)
	assert.EqualValues(t, s.CephFS, s2.CephFS)
}
