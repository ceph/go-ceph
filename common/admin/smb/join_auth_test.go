...
func TestJoinAuthIdentity(t *testing.T) {
	cluster := NewJoinAuth("ja1")
	assert.Equal(t, cluster.Type(), JoinAuthType)
	cid := cluster.Identity()
	assert.Equal(t, cid.Type(), JoinAuthType)
	assert.Equal(t, cid.String(), "ceph.smb.join.auth.ja1")
...
func TestJoinAuthSetAuth(t *testing.T) {
	ug := &JoinAuth{IntentValue: Present, AuthID: "abc"}
	ug.SetAuth("admin", "c00lt1m3")
	assert.NotNil(t, ug.Auth)
	assert.NotNil(t, ug.Auth.Username, "admin")
	assert.NotNil(t, ug.Auth.Password, "c00lt1m3")
}
