...
func (suite *RadosGWTestSuite) TestCheckBucketIndex() {
	...
	suite.T().Run("check bucket index", func(_ *testing.T) {
		resp, err := co.CheckBucketIndex(context.Background(), CheckBucketIndexRequest{Bucket: suite.bucketTestName, CheckObjects: true, Fix: true})
		assert.NoError(suite.T(), err)
		assert.Equal(suite.T(), "", resp.Index)
	})
}
