...
func (suite *RadosGWTestSuite) TestUser() {
	...
	suite.T().Run("user creation success", func(_ *testing.T) {
		usercaps := "users=read"
		user, err := co.CreateUser(context.Background(), User{ID: "leseb", DisplayName: "This is leseb", Email: "leseb@example.com", UserCaps: usercaps, OpMask: "delete", DefaultPlacement: "default-placement", DefaultStorageClass: "default-storage-class"})
		assert.NoError(suite.T(), err)
	...
	suite.T().Run("get user leseb by uid", func(_ *testing.T) {
		...
		assert.Equal(suite.T(), "default-storage-class", user.DefaultStorageClass)
