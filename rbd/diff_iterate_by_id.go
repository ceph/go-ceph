...
func (image *Image) DiffIterateByID(config DiffIterateByIDConfig) error {
	...
	if C._LIBRBD_SUPPORTS_DIFF_ITERATE3 != 1 {
		return fmt.Errorf("%w: librbd does not support rbd_diff_iterate3()", ErrNotImplemented)
	}
	...
	if config.IncludeParent == IncludeParent {
		flags |= C._RBD_DIFF_ITERATE_FLAG_INCLUDE_PARENT
	}
	if config.WholeObject == EnableWholeObject {
		flags |= C._RBD_DIFF_ITERATE_FLAG_WHOLE_OBJECT
	}
