...
//     the number of blocks and list of ChangedBlocks.
//   - and a error if any.
//
// Implements:
//
...
func (info *FileBlockDiffInfo) ReadFileBlockDiff() (*FileBlockDiffChangedBlocks, error) {
	err := info.validate()
	if err != nil {
		return nil, err
	}
	...
	cBlocks := make([]ChangedBlock, rawCephBlockDiffChangedBlocks.num_blocks)
